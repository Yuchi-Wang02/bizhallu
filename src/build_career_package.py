from __future__ import annotations

import html
import json
from pathlib import Path
from typing import Any

from public_paths import repo_path


PROJECT_ROOT = Path(__file__).resolve().parents[1]
REPORTS_DIR = PROJECT_ROOT / "reports"
DATA_DIR = PROJECT_ROOT / "data" / "processed"
RESULTS_DIR = PROJECT_ROOT / "results"
DOCS_DIR = PROJECT_ROOT / "docs"

NARRATIVE_SUMMARY_PATH = REPORTS_DIR / "bizhallu_portfolio_narrative_summary.json"
DEMO_SUMMARY_PATH = REPORTS_DIR / "bizhallu_portfolio_demo_summary.json"
INTERPRETATION_SUMMARY_PATH = REPORTS_DIR / "full100_detector_interpretation_summary.json"
QUESTION_REPORT_PATH = DATA_DIR / "business_questions_gold_report.json"
PREFLIGHT_VALIDATION_PATH = RESULTS_DIR / "full100_preflight_validation.json"

CAREER_HTML_PATH = REPORTS_DIR / "bizhallu_career_package.html"
CAREER_MD_PATH = REPORTS_DIR / "bizhallu_career_package.md"
CAREER_SUMMARY_PATH = REPORTS_DIR / "bizhallu_career_package_summary.json"


def load_json(path: Path) -> dict[str, Any]:
    return json.loads(path.read_text(encoding="utf-8"))


def esc(value: Any) -> str:
    return html.escape(str(value), quote=True)


def fmt_float(value: Any, digits: int = 3) -> str:
    return f"{float(value):.{digits}f}"


def render_bullets(items: list[str]) -> str:
    return "<ul>" + "".join(f"<li>{esc(item)}</li>" for item in items) + "</ul>"


def render_numbered(items: list[str]) -> str:
    return "<ol>" + "".join(f"<li>{esc(item)}</li>" for item in items) + "</ol>"


def render_faq(items: list[dict[str, str]]) -> str:
    return "".join(
        f"""
        <article class="faq-item">
          <h3>{esc(item["question"])}</h3>
          <p>{esc(item["answer"])}</p>
        </article>"""
        for item in items
    )


def markdown_list(items: list[str]) -> str:
    return "\n".join(f"- {item}" for item in items)


def main() -> None:
    REPORTS_DIR.mkdir(parents=True, exist_ok=True)

    narrative = load_json(NARRATIVE_SUMMARY_PATH)
    demo = load_json(DEMO_SUMMARY_PATH)
    interpretation = load_json(INTERPRETATION_SUMMARY_PATH)
    question_report = load_json(QUESTION_REPORT_PATH)
    preflight = load_json(PREFLIGHT_VALIDATION_PATH)

    positioning = narrative["positioning_statement"]
    best_auprc = interpretation["best_overall_by_test_auprc"]
    best_f1 = interpretation["best_overall_by_test_f1"]

    project_brief = [
        "Problem: business users increasingly ask LLMs to summarize revenue, rankings, returns, and product performance, but fluent answers can still bind transaction evidence incorrectly.",
        "Approach: BizHallu turns UCI Online Retail records into deterministic business questions, runs local Qwen3-0.6B generations, builds AI-assisted provisional business-fact span labels, aligns pre-identified spans to token traces, and evaluates internal-state detector signals.",
        "Result: across 103 test spans, the exploratory maximum AUPRC was 0.835 and the exploratory maximum F1 was 0.779; the maxima come from different signals selected after test comparison.",
        "Business value: the project demonstrates an auditable workflow for checking whether generated analytics claims are grounded in evidence before a stakeholder relies on them.",
        "Limitation: the 205 labels are AI-assisted and provisional, only 15 selected presentation spans received additional assistant review, claim extraction is not automated, and the detector results are not production estimates.",
    ]

    resume_bullets = [
        "Built BizHallu, an AI reliability project for business analytics that evaluates whether LLM-generated retail claims are grounded in transaction evidence.",
        "Generated 100 deterministic Online Retail questions across 7 business-analysis types and ran local Qwen3-0.6B inference with saved token-level traces.",
        "Built an AI-assisted provisional annotation workflow for 205 business-fact spans across 35 dev/test answers and aligned every pre-identified span to uncertainty and energy-style detector scores.",
        "Selected detector thresholds on dev spans and compared candidate signals on 103 test spans; exploratory maxima reached 0.835 AUPRC and 0.779 F1 from different signals.",
        "Published a GitHub Pages demo, methods page, and interview deck that connect AI evaluation with revenue reconciliation, returns, and product-concentration risk.",
    ]

    linkedin_blurb = (
        "I built BizHallu to study hallucinations in business analytics as concrete evidence-grounding failures: wrong products, ranks, amounts, countries, and conclusions. "
        "The project uses Online Retail transactions, local Qwen3-0.6B generations, AI-assisted provisional span labels, token alignment, and internal-state detector signals to show why business AI systems need auditable validation, not just fluent answers."
    )

    github_profile_blurb = (
        "Business analytics + AI reliability. I build evidence-grounded evaluation workflows for LLM-generated analysis, with a focus on finance, supply management, and decision-support use cases."
    )

    sixty_second_pitch = [
        "BizHallu is my business analytics AI reliability project.",
        "I used UCI Online Retail transactions to create deterministic questions with known gold answers, then asked Qwen3-0.6B to write concise business analysis.",
        "Instead of judging whole answers, I built an AI-assisted provisional label set for exact spans such as product names, ranks, countries, months, amounts, and percentages.",
        "The strongest result is that internal uncertainty has useful signal, but it still misses confident evidence-binding errors, especially top-3 product rank and amount mistakes.",
        "That makes the project relevant to BA, DS, and AI analyst roles because it connects data cleaning, experiment design, metrics, and business-risk interpretation.",
    ]

    five_minute_flow = [
        "Start with the business problem: a stakeholder may ask an LLM which product, country, or month performed best, and a fluent answer can still be wrong.",
        "Explain the data pipeline: Online Retail records are cleaned into deterministic evidence tables and gold answers, so each question can be audited.",
        "Explain the model run: Qwen3-0.6B is small enough to run locally, which makes the project reproducible and transparent.",
        "Explain the label unit: the evaluation unit is a pre-identified business fact span, not a whole answer; automatic claim extraction is not part of the current pipeline.",
        "Explain the metrics honestly: thresholds are selected on dev spans, but the headline AUPRC/F1 signals were chosen after test comparison, so the values are exploratory maxima rather than confirmatory held-out estimates.",
        "Show q_0064 or q_0069: the model uses plausible values but binds them to the wrong rank or product, which is exactly the business-risk failure mode.",
        "Close with the next step: add evidence-aware claim verification to complement internal uncertainty signals.",
    ]

    faq = [
        {
            "question": "Why is this relevant to business analytics instead of only NLP research?",
            "answer": "The errors are business facts: wrong revenue, country, product, month, rank, percentage, or conclusion. Those are exactly the claims a business user might put into a decision memo.",
        },
        {
            "question": "Why span-level labels?",
            "answer": "Whole-answer labels hide mixed cases. A generated answer can correctly state rank 1 and rank 2 while hallucinating rank 3, so the useful unit is the exact fact span.",
        },
        {
            "question": "Why Qwen3-0.6B?",
            "answer": "It is small enough to run locally on available hardware while still producing realistic fluent errors. That makes the experiment reproducible and portfolio-friendly.",
        },
        {
            "question": "Why not claim production-ready hallucination detection?",
            "answer": "The current detector baselines are diagnostic. They show signal and limitations, but the dataset and label scope are not large enough for a production claim.",
        },
        {
            "question": "Why did simple uncertainty beat the energy-family result?",
            "answer": "In this run, the strongest energy-family F1 came from a probability-mass control, while simple entropy and top-2 margin were slightly stronger overall. That is a result, not a problem.",
        },
        {
            "question": "What is the strongest technical decision?",
            "answer": "Thresholds are selected on dev spans and reused on test spans. However, the headline signal for each metric was chosen after comparing test results, so I present 0.835 and 0.779 as exploratory maxima and would use a fresh confirmation set for a confirmatory claim.",
        },
        {
            "question": "What is the strongest business insight?",
            "answer": "LLMs can copy real-looking values from a table while assigning them to the wrong product or rank. Business AI needs evidence checks, not only answer fluency checks.",
        },
        {
            "question": "How does your accounting and supply-management background show up?",
            "answer": "The project focuses on net revenue, returns, product performance, country exposure, and rank-based reporting. These connect to accounting reconciliation and supply-management thinking about product concentration, while avoiding inventory, cost, or vendor claims that the dataset cannot support.",
        },
        {
            "question": "What would you build next?",
            "answer": "First, define an independent claim-extraction and claim-evidence decision protocol. Then compare that evidence-aware verifier with entropy, top-2 margin, energy-style signals, and selected literature baselines on a fresh evaluation design.",
        },
        {
            "question": "How should the label scope be described?",
            "answer": "Describe 205 AI-assisted provisional span labels and 15 selected presentation spans with additional assistant review. There is no independent human annotation or inter-annotator agreement.",
        },
    ]

    guardrails = [
        "Say span-level business fact evaluation, not whole-answer correctness.",
        "Say 205 AI-assisted provisional labels and 15 additionally assistant-reviewed presentation spans, not a human-labeled benchmark.",
        "Say diagnostic detector baselines, not production-ready detection.",
        "Say simple uncertainty is strongest in this run; do not imply energy baselines won overall.",
        "Say the 0.835 AUPRC and 0.779 F1 values are exploratory test maxima from different signals.",
    ]

    links = [
        ("Live demo", "https://yuchi-wang02.github.io/bizhallu/"),
        ("Interactive case demo", "https://yuchi-wang02.github.io/bizhallu/portfolio_demo.html"),
        ("Detector interpretation", "https://yuchi-wang02.github.io/bizhallu/detector_interpretation.html"),
        ("Interview deck", "https://yuchi-wang02.github.io/bizhallu/assets/bizhallu_ai_reliability_deck.pptx"),
        ("JHU Carey BAAI", "https://carey.jhu.edu/programs/master-science/business-analytics-artificial-intelligence/full-time"),
        ("BLS Data Scientists", "https://www.bls.gov/ooh/math/data-scientists.htm"),
        ("O*NET Data Scientists", "https://www.onetonline.org/link/summary/15-2051.00"),
        ("O*NET Business Intelligence Analysts", "https://www.onetonline.org/link/summary/15-2051.01"),
    ]

    summary = {
        "status": "career_package_ready",
        "career_html_path": repo_path(CAREER_HTML_PATH),
        "career_markdown_path": repo_path(CAREER_MD_PATH),
        "question_count": narrative["question_count"],
        "question_type_count": len(question_report["question_type_counts"]),
        "annotated_span_count": narrative["annotated_span_count"],
        "heldout_test_span_count": 103,
        "best_test_auprc": best_auprc["test_auprc"],
        "best_test_auprc_baseline": best_auprc["baseline"],
        "best_test_f1": best_f1["test_f1"],
        "best_test_f1_baseline": best_f1["baseline"],
        "primary_question_ids": demo["primary_question_ids"],
        "label_lock_basis": narrative["label_lock_basis"],
        "provisional_annotation_count": narrative["annotated_span_count"],
        "presentation_reviewed_span_count": 15,
        "independent_human_annotation": False,
        "automatic_claim_extraction": False,
        "metric_selection_status": "exploratory_test_maxima",
        "resume_bullet_count": len(resume_bullets),
        "faq_count": len(faq),
        "guardrail_count": len(guardrails),
        "ready_for_current_stage": preflight["ready_for_current_stage"],
        "num_failures": 0,
        "failures": [],
    }

    link_html = "".join(
        f'<a class="artifact-link" href="{esc(url)}">{esc(label)}</a>' for label, url in links
    )
    faq_html = render_faq(faq)

    html_text = f"""<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>BizHallu Career Package</title>
    <style>
      :root {{
        --bg: #f6f7f9;
        --surface: #ffffff;
        --ink: #1d1d1f;
        --muted: #5f6368;
        --line: rgba(29, 29, 31, 0.13);
        --blue: #0066cc;
        --green: #0f766e;
        --amber: #9a6700;
      }}
      * {{ box-sizing: border-box; }}
      html {{ scroll-behavior: smooth; }}
      body {{
        margin: 0;
        background: linear-gradient(180deg, #ffffff 0%, var(--bg) 50%, #eef2f5 100%);
        color: var(--ink);
        font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Arial, sans-serif;
        line-height: 1.55;
      }}
      a {{ color: inherit; text-decoration: none; }}
      .topbar {{
        position: sticky;
        top: 0;
        z-index: 10;
        min-height: 62px;
        padding: 0 34px;
        display: flex;
        align-items: center;
        justify-content: space-between;
        border-bottom: 1px solid var(--line);
        background: rgba(246, 247, 249, 0.88);
        backdrop-filter: blur(16px);
      }}
      .brand {{ display: flex; align-items: center; gap: 10px; font-weight: 850; }}
      .brand span {{
        display: grid;
        place-items: center;
        width: 32px;
        height: 32px;
        border-radius: 8px;
        background: var(--ink);
        color: white;
        font-size: 12px;
      }}
      nav {{ display: flex; gap: 14px; color: var(--muted); font-size: 14px; font-weight: 750; }}
      main {{ width: min(1160px, calc(100% - 40px)); margin: 0 auto; }}
      section {{ padding: 58px 0; border-top: 1px solid var(--line); }}
      .hero {{
        min-height: calc(100vh - 62px);
        display: grid;
        grid-template-columns: minmax(0, 1.15fr) minmax(300px, 0.85fr);
        gap: 34px;
        align-items: center;
        border-top: 0;
      }}
      .eyebrow {{
        margin: 0 0 12px;
        color: var(--blue);
        font-size: 12px;
        font-weight: 850;
        letter-spacing: 0;
        text-transform: uppercase;
      }}
      h1, h2, h3, p, li {{ overflow-wrap: anywhere; }}
      h1 {{ margin: 0; max-width: 860px; font-size: clamp(42px, 6vw, 70px); line-height: 1; letter-spacing: 0; }}
      h2 {{ margin: 0; font-size: clamp(30px, 4vw, 42px); line-height: 1.1; letter-spacing: 0; }}
      h3 {{ margin: 0; font-size: 19px; line-height: 1.22; letter-spacing: 0; }}
      .lede {{ margin: 22px 0 0; max-width: 820px; color: var(--muted); font-size: 21px; }}
      .snapshot, .panel, .faq-item, .metric-card {{
        border: 1px solid var(--line);
        border-radius: 8px;
        background: var(--surface);
      }}
      .snapshot {{ display: grid; overflow: hidden; }}
      .snapshot div {{ padding: 20px; border-bottom: 1px solid var(--line); }}
      .snapshot div:last-child {{ border-bottom: 0; }}
      .label {{ display: block; color: var(--muted); font-size: 12px; font-weight: 850; text-transform: uppercase; }}
      .snapshot strong {{ display: block; margin-top: 6px; font-size: 20px; }}
      .grid {{ display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 14px; margin-top: 26px; }}
      .metric-grid {{ display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 12px; margin-top: 28px; }}
      .panel, .faq-item, .metric-card {{ padding: 22px; }}
      .metric-card strong {{ display: block; margin-top: 8px; font-size: 32px; line-height: 1; }}
      .metric-card p, .panel p, .faq-item p, li {{ color: var(--muted); }}
      ul, ol {{ display: grid; gap: 10px; margin: 18px 0 0; padding-left: 20px; }}
      .faq-grid {{ display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px; margin-top: 26px; }}
      .quote {{
        margin-top: 24px;
        padding: 22px;
        border-left: 4px solid var(--blue);
        background: rgba(0, 102, 204, 0.07);
        border-radius: 8px;
        font-size: 20px;
        font-weight: 750;
      }}
      .artifact-row {{ display: flex; flex-wrap: wrap; gap: 10px; margin-top: 22px; }}
      .artifact-link {{
        display: inline-flex;
        align-items: center;
        min-height: 40px;
        padding: 9px 13px;
        border: 1px solid rgba(0, 102, 204, 0.24);
        border-radius: 8px;
        color: var(--blue);
        background: rgba(0, 102, 204, 0.07);
        font-weight: 800;
        font-size: 14px;
      }}
      code {{
        padding: 2px 5px;
        border-radius: 6px;
        background: rgba(0,0,0,0.06);
        font-family: "SFMono-Regular", Consolas, monospace;
      }}
      footer {{ padding: 34px 0 50px; color: var(--muted); border-top: 1px solid var(--line); }}
      @media (max-width: 900px) {{
        .topbar {{ padding: 0 20px; }}
        nav {{ display: none; }}
        main {{ width: min(100% - 28px, 720px); }}
        .hero, .grid, .faq-grid, .metric-grid {{ grid-template-columns: 1fr; }}
        .hero {{ min-height: auto; padding-top: 46px; }}
      }}
    </style>
  </head>
  <body>
    <header class="topbar">
      <a class="brand" href="./index.html"><span>BH</span>BizHallu</a>
      <nav>
        <a href="#brief">Brief</a>
        <a href="#pitch">Pitch</a>
        <a href="#faq">FAQ</a>
        <a href="#guardrails">Guardrails</a>
      </nav>
    </header>

    <main>
      <section class="hero">
        <div>
          <p class="eyebrow">Career package for BA / DS / AI analyst roles</p>
          <h1>Business analytics systems should audit their own evidence.</h1>
          <p class="lede">{esc(positioning)}</p>
          <div class="quote">Use BizHallu as an interview story about data quality, LLM evaluation, metrics discipline, and business-risk interpretation.</div>
        </div>
        <aside class="snapshot">
          <div><span class="label">Gold questions</span><strong>{summary["question_count"]}</strong></div>
          <div><span class="label">Question types</span><strong>{summary["question_type_count"]}</strong></div>
          <div><span class="label">Provisional aligned spans</span><strong>{summary["annotated_span_count"]}</strong></div>
          <div><span class="label">Exploratory test maxima</span><strong>{fmt_float(summary["best_test_auprc"])} AUPRC / {fmt_float(summary["best_test_f1"])} F1</strong></div>
          <div><span class="label">Primary demo cases</span><strong>{esc(" / ".join(summary["primary_question_ids"]))}</strong></div>
        </aside>
      </section>

      <section id="brief">
        <p class="eyebrow">One-page project brief</p>
        <h2>What a recruiter or interviewer should understand first.</h2>
        <div class="grid">
          <article class="panel">{render_bullets(project_brief)}</article>
          <article class="panel">
            <h3>Public artifacts</h3>
            <p>These links are safe to share in a resume, GitHub profile, LinkedIn post, or interview follow-up.</p>
            <div class="artifact-row">{link_html}</div>
          </article>
        </div>
      </section>

      <section id="pitch">
        <p class="eyebrow">Interview language</p>
        <h2>Prepare one short version and one technical version.</h2>
        <div class="grid">
          <article class="panel">
            <h3>60-second version</h3>
            {render_numbered(sixty_second_pitch)}
          </article>
          <article class="panel">
            <h3>5-minute version</h3>
            {render_numbered(five_minute_flow)}
          </article>
        </div>
      </section>

      <section id="resume">
        <p class="eyebrow">Reusable wording</p>
        <h2>Resume bullets and profile copy.</h2>
        <div class="grid">
          <article class="panel">
            <h3>Resume bullets</h3>
            {render_bullets(resume_bullets)}
          </article>
          <article class="panel">
            <h3>LinkedIn / GitHub profile</h3>
            <p>{esc(linkedin_blurb)}</p>
            <p><strong>Short profile line:</strong> {esc(github_profile_blurb)}</p>
          </article>
        </div>
      </section>

      <section id="faq">
        <p class="eyebrow">Interview FAQ</p>
        <h2>Answers to the questions this project should invite.</h2>
        <div class="faq-grid">{faq_html}</div>
      </section>

      <section id="guardrails">
        <p class="eyebrow">Claim guardrails</p>
        <h2>Keep the public story strong by keeping it precise.</h2>
        <div class="panel">
          {render_bullets(guardrails)}
        </div>
      </section>
    </main>

    <footer>
      <main>
        Career package generated from validated BizHallu reports. Label basis: <code>{esc(summary["label_lock_basis"])}</code>.
      </main>
    </footer>
  </body>
</html>
"""

    md_text = f"""# BizHallu Career Package

## Positioning

{positioning}

## Project Brief

{markdown_list(project_brief)}

## Resume Bullets

{markdown_list(resume_bullets)}

## LinkedIn / GitHub Blurb

{linkedin_blurb}

Short profile line: {github_profile_blurb}

## 60-Second Pitch

{markdown_list(sixty_second_pitch)}

## 5-Minute Interview Flow

{markdown_list(five_minute_flow)}

## Interview FAQ

{chr(10).join(f"- {item['question']} {item['answer']}" for item in faq)}

## Public Claim Guardrails

{markdown_list(guardrails)}

## Public Links

{chr(10).join(f"- [{label}]({url})" for label, url in links)}
"""

    CAREER_HTML_PATH.write_text(html_text, encoding="utf-8")
    CAREER_MD_PATH.write_text(md_text, encoding="utf-8")
    CAREER_SUMMARY_PATH.write_text(json.dumps(summary, indent=2, ensure_ascii=True), encoding="utf-8")

    print(json.dumps({"status": summary["status"], "html": repo_path(CAREER_HTML_PATH)}, indent=2))


if __name__ == "__main__":
    main()
