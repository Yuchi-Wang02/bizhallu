...
PROMPTS_PATH = PROJECT_ROOT / "outputs" / "qwen_top3_structured_prompts.jsonl"
REPORT_PATH = PROJECT_ROOT / "outputs" / "qwen_top3_structured_prompts_report.json"
VALIDATION_PATH = PROJECT_ROOT / "outputs" / "qwen_top3_structured_prompts_validation.json"
PILOT3_CONFIG_PATH = PROJECT_ROOT / "configs" / "top3_structured_pilot3_questions.json"

EXPECTED_VARIANT_ID = "top3_structured_v0.1"
...
def main() -> None:
    ...
    if not PROMPTS_PATH.exists():
        add_failure(failures, "GLOBAL", "missing structured prompt jsonl", str(PROMPTS_PATH))
    if not REPORT_PATH.exists():
        add_failure(failures, "GLOBAL", "missing structured prompt report", str(REPORT_PATH))
    if not PILOT3_CONFIG_PATH.exists():
        add_failure(failures, "GLOBAL", "missing pilot3 config", str(PILOT3_CONFIG_PATH))
    ...
    if not isinstance(pilot3_question_ids, list) or not all(isinstance(item, str) for item in pilot3_question_ids):
        add_failure(failures, "GLOBAL", "pilot3 config question_ids must be a string list")
        pilot3_question_ids = []
    if set(pilot3_question_ids) - prompt_question_ids:
        add_failure(failures, "GLOBAL", "pilot3 config contains question ids missing from structured prompts", sorted(set(pilot3_question_ids) - prompt_question_ids))
    if pilot3_config.get("source_prompt_file") != "outputs/qwen_top3_structured_prompts.jsonl":
    ...
    if "--prompts-path outputs/qwen_top3_structured_prompts.jsonl" not in planned_command:
        add_failure(failures, "GLOBAL", "pilot3 planned command missing structured prompts path")
    ...
    for record in prompts:
        ...
        if not prompt_id.endswith("_top3_structured"):
            add_failure(failures, prompt_id, "prompt_id missing structured suffix")
        ...
        if record["row_order_policy"] != "deterministic_hash_by_question_and_product":
            add_failure(failures, prompt_id, "row order policy changed unexpectedly")
        ...
        for phrase in required_phrases:
            if phrase not in full_prompt:
                add_failure(failures, prompt_id, "missing structured instruction", phrase)
        ...
        if stock_codes(evidence_rows) != stock_codes(question["evidence"]["rows"]):
            # The variant should preserve the same deterministic order used by the base prompt,
            # not the source order. Compare as a set here and check anti-leakage below.
        ...
        if net_revenues(evidence_rows) == sorted(net_revenues(evidence_rows), reverse=True):
            add_failure(failures, prompt_id, "evidence rows are already sorted by answer metric")

        expected_output = record["expected_output_rows_for_validation_only"]
        if [str(row["stock_code"]) for row in expected_output] != expected_top3_from_evidence(question["evidence"]["rows"]):
